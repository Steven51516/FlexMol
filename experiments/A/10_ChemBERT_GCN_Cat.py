...
for i in range(num):
    ...
    torch.manual_seed(i)
    FM = FlexMol()
    de = FM.init_drug_encoder("ChemBERTa", output_dim = 64) 
    pe = FM.init_prot_encoder("GCN", pdb=True, data_dir = dir + "pdb/", output_feats = 64)
    ...
    while(os.path.exists('./result/'+name+ '/10/'+str(now) + '/')):
        now=now+1
    trainer.metrics_dir = './result/'+name+ '/10/'+str(now) + '/'
    trainer.test(test)