...
for i in range(num):
    ...
    torch.manual_seed(i)
    FM = FlexMol()
    de = FM.init_drug_encoder("GCN", output_feats = 64) 
    pe = FM.init_prot_encoder("GCN_ESM", pickle_dir = '/root/FlexMol/pickles/davis/esm_graph', pdb=True, data_dir = dir + "pdb/", output_feats = 64, hidden_feats=[128, 128, 128])
    ...
    while(os.path.exists('./result/'+name+ '/9/'+str(now) + '/')):
        now=now+1
    trainer.metrics_dir = './result/'+name+ '/9/'+str(now) + '/'
    trainer.test(test)