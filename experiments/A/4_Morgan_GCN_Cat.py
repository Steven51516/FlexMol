...
for i in range(num):
    ...
    torch.manual_seed(i)
    FM = FlexMol()
    de = FM.init_drug_encoder("Morgan", output_feats = 64) 
    ...
    while(os.path.exists('./result/'+name+ '/4/'+str(now) + '/')):
        now=now+1
    trainer.metrics_dir = './result/'+name+ '/4/'+str(now) + '/'
    trainer.test(test)
