# name:3_CNN_Morgan_CNN_GCN_Cat.py
import sys
import os
...
for i in range(num):
    ...
    torch.manual_seed(i)
    FM = FlexMol()
    de1 = FM.init_drug_encoder("Daylight", output_dim = 128) 
    de2 = FM.init_drug_encoder("CNN", output_feats = 128) 
    
    pe = FM.init_prot_encoder("CNN", output_feats = 128)
    ...
    while(os.path.exists('./result/'+name+ '/3/'+str(now) + '/')):
        now=now+1
    trainer.metrics_dir = './result/'+name+ '/3/'+str(now) + '/'
    trainer.test(test)
