#name : 13_Transformer_Transformer_Cat
import sys
import os
...
for i in range(num):
    ...
    torch.manual_seed(i)
    FM = FlexMol()
    de = FM.init_drug_encoder("Transformer") 
    pe = FM.init_prot_encoder("Transformer")
    ...
    while(os.path.exists('./result/'+name+ '/13/'+str(now) + '/')):
        now=now+1
    trainer.metrics_dir = './result/'+name+ '/13/'+str(now) + '/'
    trainer.test(test)
