# name:6_GCN_Transformer_AAC_GAT_Cat.py
import sys
import os
...
for i in range(num):
    ...
    torch.manual_seed(i)
    FM = FlexMol()
    de1 = FM.init_drug_encoder("Morgan", output_feats = 64) 
    de2 = FM.init_drug_encoder("GCN", output_feats = 64) 
    pe = FM.init_prot_encoder("GCN", pdb=True, data_dir = dir + "pdb/",  output_feats=64, pickle_dir = '/root/FlexMol/pickles/davis/protein_graph/')

    dp = FM.set_interaction([de1, de2, pe], "cat")
    ...
    while(os.path.exists('./result/'+name+ '/6/'+str(now) + '/')):
        now=now+1
    trainer.metrics_dir = './result/'+name+ '/6/'+str(now) + '/'
    trainer.test(test)